...
public class ActorDiscoverExample {
    ...
    static class PingService {
        ...
        public static final ServiceKey<Ping> pingServiceKey = ServiceKey.create(Ping.class, "pingService");
    ...
    static class Pinger {
        ...
        public static Behavior<PingService.Pong> create(ActorRef<PingService.Ping> pingService) {
            return Behaviors.setup(context -> {
                pingService.tell(new PingService.Ping(context.getSelf()));
                return new Pinger(context, pingService).behavior();
            });
        }
    ...
    // Subscribing with Receptionist
    static class Guardian {
        private static Behavior<Void> create() {
            return Behaviors.setup((ActorContext<Receptionist.Listing> context) -> {
                context.getSystem().receptionist()
                        .tell(Receptionist.subscribe(PingService.pingServiceKey, context.getSelf()));
        ...
        private Behavior<Receptionist.Listing> onListing(Receptionist.Listing msg) {
            msg.getAllServiceInstances(PingService.pingServiceKey)
                    .forEach(pingService -> {
                        context.spawnAnonymous(Pinger.create(pingService));
                    });
            return Behaviors.same();
        }
    }

    // Finding with Receptionist
    ...
    public static void main(String[] args) {
       ...
       ActorRef<Void> actorRef= ActorSystem.create(Guardian.create(),"guardian");
//        ActorRef<PingManager.Command> actorRef = ActorSystem.create(PingManager.create(), "pingManager");
//        actorRef.tell(PingManager.PingAll.INSTANCE);
