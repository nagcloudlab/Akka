...
public class AsyncHttpServerWithRoutes {
    ...
    Function<HttpRequest, CompletionStage<HttpResponse>> getAllCustomers = (request) -> {
        CompletableFuture<HttpResponse> response = new CompletableFuture<>();
        CompletionStage<List<Customer>> customersFuture = AskPattern.ask(actorSystem, me -> new CustomerBehavior.GetCustomersCommand(me), Duration.ofSeconds(5), actorSystem.scheduler());
